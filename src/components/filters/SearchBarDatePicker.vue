<template>
  <div class="hold-filter--check">
    ...
    >
      ...
      <!-- <p>{{ this.selectedDuration }}</p> -->
        ...
        <div style="margin-left: auto">
          <button :class="dateRange.endDate ? '' : 'disabled'" @click="onClear">Clear</button>

          <button @click="data.clickApply" :class="(filterBy.citySelected && filterBy.citySelected.slug === 'lisbon') && durationDays < 30 ? 'date-apply-filter date-apply-disabled' : 'date-apply-filter'">Apply filters</button>
    ...
    <div v-show="!isSelect" class="mobile_only mobile-flow-footer bg--light q_flex">
      ...
      <button
        class="bt-primary"
        @click="$store.commit('setCurrentMobilePopUp', 'guest'), updateDate"
        :class="(filterBy.citySelected && filterBy.citySelected.slug === 'lisbon') && durationDays < 30 ? 'date-apply-disabled' : ''"
      >
        Next
      </button>
    </div>
...
export default {
  name: "SearchBarDatePickerFilter",
  ...
  methods: {
    getDate(val) {
      // Call getDuration as user hovers over dates
      this.getDuration(val)
      if(this.customRef === 'homePagePicker' || this.customRef === 'wizardPicker') {
        return (
          dayjs(val)
            .format("ddd, MMM DD")
        );
    ...
    getDuration() {
      ...
      } else if(this.$refs.checkoutPicker) {
